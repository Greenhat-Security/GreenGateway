...
- Request admission and authentication now enforce the bounds the policy kernel evaluates under, so no request the gateway serves can later be refused at authorization for its shape (issue #488). Three limits are new at admission, each with its own status: a request path longer than `MAX_REQUEST_PATH_BYTES` (default and ceiling 8192, the kernel's own bound) is `414 URI Too Long`; an HTTP method longer than 64 bytes is `501 Not Implemented`; and the `Host` header is validated as `uri-host [":" port]` -- a repeated `Host`, a value that is not visible ASCII, a bracketed value that is not an IPv6 literal, a port that is not a port, or a character RFC 3986 does not allow in a host is `400 Bad Request`, and a host longer than 4096 bytes is `431 Request Header Fields Too Large`. Previously `[a:b:c]` was read as the host `a:b:c` and `host:x` as `host`; both are now malformed input, and a rejected `Host` is never echoed. Principal claims are bounded at authentication by every validator -- at most 256 roles, each at most 256 bytes and none empty, and a subject of at most 4096 bytes -- and a credential outside them is `401` before any revocation lookup, with the `auth.failure` reason naming the bound; an empty role name, which could never match a policy role, is refused alongside the existing empty-subject check. Service-token scopes are held to the same bounds when a token is created (`400`) and when it authenticates (`401`). None of these limits was enforced before: an 8 KiB path or a 300-role token was authorized normally, which the pending kernel cutover (#422) would have turned into a failure at authorization time.
