...
Every result envelope exposes schema/evaluator versions, pinned revisions and digests, mode, declared completeness domain, outcome, enforcement, effective action, completeness, limitation codes, terminal reason, stable matched identifiers, bounded stage entries, and applied bounds. Stage status is `matched`, `skipped`, `not_applicable`, `unknown`, or `not_evaluated`; excluded live stages are always explicit. List and detail data are separately paginated and permissioned.
...
Strict-versioned synthetic test fixtures are the sole persistence exception because #243 requires deterministic cases stored beside policy. They persist only bounded, secret-free typed values under owner-scoped test permissions, strong ETags, stable IDs, canonical digests, explicit lifecycle/retention, and the protected policy-resource storage boundary. Authorized test CRUD may return fixtures; results, traces, audits, metrics, evidence, exports, URLs, and browser storage expose only identifiers, digests, expectations, and sanitized outcomes. Raw HTTP bodies, tool results, serialized production principals, and raw source events are never accepted merely to improve analysis. Existing retained fields such as source IP, request ID, user agent, path, and actor data must pass a centralized purpose-specific projection before leaving the audit boundary.

Every draft, test revision, job, run result, detail projection, and evidence artifact has a positive retention/TTL limit, bounded cleanup, and observable expiry. Later slices choose tested defaults; configuration can lower but not exceed hard ceilings. Existing audit retention remains the source-data compatibility anchor and is never extended implicitly. Expiry or pruning produces an explicit expired, unavailable, or incomplete state rather than an empty success.
