...
The current OIDC binding cookie is a **login transaction cookie**, not an authenticated admin session. GreenGateway currently has no OIDC session-issuance or logout endpoint. Externally issued cookies continue to use the configured `cookie_session` introspection provider and its logout/revocation mechanism. Routine OIDC cookie sessions and their logout contract remain tracked in #424; the UI must not advertise them until that server contract exists. Existing cookie flags, configurable CSRF names, redirect origin validation, custom admin prefixes and management listener boundaries are unchanged. In-memory tokens reduce persistence but cannot protect against code executing in the current browser origin.
