...
SSO completion requires the browser that started the login. Keep the public admin UI and the configured callback on the same origin, use HTTPS in production, and allow the UI's JSON POST to the callback path through your reverse proxy. The gateway sets a short-lived HttpOnly transaction cookie; completion requires that cookie and returns the access token only in a one-use POST response. Copied completion links and legacy token fragments cannot sign in another browser. See [the login flow and upgrade notes](../configuration.md#admin_login_provider).
