...
Format and validation: a valid Unicode filesystem path that exists and canonicalizes to a directory at startup. File aliases are restricted to one validated filename segment under this canonical root. Every resolution rejects symbolic links and Windows reparse points, opens the leaf without following links, requires a regular file, and caps the read before material is parsed. On Unix, the root must not be group/other writable and secret files must grant no group/other permissions. Windows enforces the regular-file and reparse-point boundary, but ACL ownership/readability policy remains an operator and deployment-platform responsibility.
