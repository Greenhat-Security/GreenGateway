...
export function TrafficOverview() {
  ...
  return <section className="panel traffic-overview" aria-labelledby="traffic-overview-heading">
  ...
  return <>
    ...
    {endpoints.length > 0 && <>
      <div className="flow-stats" aria-label="Loaded traffic summary">
        ...
        <div><span>Mixed / unknown coverage</span><strong>{number(unknown)}</strong></div>
      </div>
      ...
      <div className="flow-legend"><span><i className="covered" />Rule covered</span><span><i className="uncovered" />No matching rule</span><span><i className="unknown" />Mixed / unknown</span></div>
      <div className="flow-destinations">{groups.map(group => <button type="button" key={group.name} className="flow-destination" aria-expanded={selected === group.name}
        ...
        <span className="flow-distribution"><strong>{number(group.total)} requests</strong><span className="flow-stacked" aria-hidden="true">{(['covered', 'uncovered', 'unknown'] as const).map(key => <span key={key} className={key} style={{ width: `${group[key] / group.total * 100}%` }} />)}</span>
          <small>{number(group.covered)} covered · {number(group.uncovered)} no rule · {number(group.unknown)} mixed / unknown</small></span><span aria-hidden="true">{selected === group.name ? '−' : '+'}</span></button>)}</div>
...
function Sankey({ flows }: { flows: Flow[] }) {
  ...
  return <div className="flow-canvas"><svg viewBox={`0 0 960 ${diagram.height}`} role="img" aria-labelledby={`${id}-title ${id}-description`}>
