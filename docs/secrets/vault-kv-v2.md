...
`workload_jwt` is the recommended mechanism and needs no bootstrap secret at all. It reads a projected workload token from a directory that GreenGateway canonicalizes and holds as a capability handle at startup, so replacing the path or an ancestor afterwards cannot redirect the read, and it posts that token to a fixed auth mount and role. The read opens the leaf in nonblocking mode, revalidates the opened handle as a regular file, and caps the read before any material is parsed.

Two allowances exist so that the shapes container runtimes actually publish can be read, and both stay confined to the pinned capability root. The kubelet materializes each projected file as a relative symlink into a timestamped `..data` directory, so the leaf may be a symlink and is resolved beneath that root — a link pointing outside it still fails closed. The kubelet also mounts the projected volume as a tmpfs whose root is `drwxrwxrwt` (mode `1777`), so a group- or other-writable root is accepted when the sticky bit is set, which is what prevents a process that does not own an entry from renaming or deleting it; a writable root *without* the sticky bit is rejected. Group and other *read* permissions are tolerated on the token file itself, because that is how projected tokens are published, while group or other *write* on the file still fails closed.
