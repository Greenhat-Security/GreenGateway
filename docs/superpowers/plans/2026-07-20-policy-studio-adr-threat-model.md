...
Add `### Side-effect boundary` and explicitly prohibit every simulation, replay, test, analyzer, optimizer, and evidence-assembly path from DNS, HTTP, MCP/provider/secret-store calls, token or credential validation, tool invocation, semaphore acquisition, production bucket reads or writes, policy/suggestion mutation, physical upstream selection, and normal data-plane authz event emission. Keep optional protected signing as a separate authorized post-assembly branch that may invoke only the #240 signer contract.
...
- Approved bounded non-secret matcher inputs for ad hoc simulations (allowlisted headers, query values, typed identity attributes, validated tool arguments) may exist transiently only in authorized page memory before submission, the authenticated request, and evaluator memory, then are discarded.
