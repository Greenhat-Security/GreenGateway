...
- **Registration commits through the authority.** `POST /v1/admin/tools/openapi/register` reads the authoritative document and ETag, merges, validates against the replica's current lanes, commits the new immutable version under the request's `If-Match` (a racing writer loses with `412`), and installs the lane. A commit that cannot compile locally (for example, a name collision with a managed lane that moved underneath it) is durable at the authority and fails closed on that replica until reconciliation resolves or surfaces the conflict.
- **The managed-connection catalog and refresh surfaces** (`CONNECTIONS_SQLITE_PATH`) are still local-only and rejected in cluster mode; their PostgreSQL control plane is the remaining half of PR 8.
