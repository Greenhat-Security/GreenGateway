...
`GET /version` includes `admin_session` only when this capability is configured, independently of `admin_login_configured`. It reports `storage`, `completion_mode`, the prefix-derived `login_url`, `completion_url`, `logout_url`, and `max_age_seconds`. After the existing browser-bound login/callback, POST completion with `{"code":"...","state":"...","mode":"cookie"}` and the configured CSRF pair. Successful completion sets the HttpOnly session cookie and returns only `{"authenticated":true,"expires_in":3600}`; no bearer credential reaches browser JavaScript. Missing `mode` or `mode:"bearer"` retains the existing one-time `{access_token}` response and automation bearer compatibility. Unsupported cookie completion is rejected before consuming the pending login.
