...
import { destinationLabel, flowDiagram, summarizeDestinations, trafficFlows, UNKNOWN_DESTINATION } from './trafficFlow';
...
describe('traffic flow accounting', () => {
  ...
  it('uses origins and never exposes credentials, query strings, or URL paths', () => {
    ...
    expect(destinationLabel(null)).toBe(UNKNOWN_DESTINATION);
  });
});
