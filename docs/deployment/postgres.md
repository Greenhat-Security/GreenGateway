...
- **No serving replica writes a durable audit row.** In cluster mode a replica's audit sinks are stdout, the optional log file, and the in-process broadcast (`gateway/src/audit/sink.rs`); `greengateway.audit_events` is written by `gateway import-standalone`, read by the durable SSE stream, the discovery projector and cluster-mode suggestion generation, and pruned by the maintenance singleton. Nothing on the request path inserts into it. So the stream guarantees above are guarantees about the store and its readers, and a cluster deployment's audit of record today is the replicas' stdout, collected by whatever ships their logs. `saturation.rs`'s `sustained_request_load_never_loses_an_audit_record_silently` measures the in-process queue and its drop counter, which is the bound that does exist. `performance.rs`'s `the_audit_enqueue_stays_off_the_request_path` asserts the durable row count is zero as a tripwire — but it is `#[ignore]`d and `ha_performance` is not a leg of the release gate, so **that tripwire cannot fail a pull request**. It fails the scheduled nightly, whose failures are deliberately not read as a broken pull request. Treat this paragraph as verified by review, and re-check it in the PR that wires a durable sink rather than expecting a red check to do it for you.
- **No strict audit mode.** There is no setting that makes a saturated audit queue unready, and no queue-depth arm in the cluster status view's `degraded` reasons. Saturation is observable and bounded and the data plane keeps serving; alert on `greengateway_audit_queue_oldest_age_seconds` and `audit_events_dropped_total` (that one predates the `greengateway_` prefix and is emitted without it), not on a state change that does not exist.
