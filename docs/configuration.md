...
- `connect_timeout_ms`: budget for establishing an upstream connection and acquiring stream capacity on an existing one. Default `10000`, range `100`-`60000`. This is not the caller's deadline: exceeding it is `UNAVAILABLE`, not `DEADLINE_EXCEEDED`.
- `idle_timeout_ms`: terminates a stream with no upstream-to-client traffic. Default `300000`, range `1000`-`3600000`. Zero disables it.
- `max_duration_ms`: ceiling on one call's total duration, and the cap applied to a client's `grpc-timeout`. Default `3600000` (one hour), maximum `604800000` (seven days). Zero disables the ceiling, in which case an uncapped client deadline is honoured.
...
Authentication, rate limiting, CSRF, request validation, RBAC, direct policy, canonical method validation, and admission all complete before any DNS resolution, connection acquisition, or upstream byte. A denied call produces a protocol-correct trailers-only gRPC response and contacts no upstream. Full deployment guidance, including the front-end requirement and where gRPC cannot work at all, is in [docs/deployment/grpc.md](deployment/grpc.md).
