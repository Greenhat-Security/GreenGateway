...
Authentication validity, CSRF, request-schema validation, mutable rate-bucket capacity, semaphore availability, DNS resolution, transport health, physical upstream selection, and upstream execution stay outside the pure verdict. Every simulation, replay, test, analyzer, optimizer, and evidence-assembly caller performs no DNS, network, provider, secret-store, credential-validation, tool-invocation, production-bucket, semaphore, policy-mutation, or ordinary data-plane audit side effect. Optional protected signing is a separate authorized branch after complete assembly and may invoke only the #240 signer contract. Results expose every excluded live stage as `not_evaluated`; `complete` is scoped to the declared pinned policy-evaluation domain, and `would_forward` never claims end-to-end live success.
