...
| Principals/reviews/signal and suggestion transitions/suggestion generation | Local working sets + SQLite snapshots | Reviews and signal transitions write the shared tables today (last writer wins, as standalone); expected-state predicates, atomic suggestion acceptance, and cluster-mode suggestion generation arrive with PR 12 | Transitions become CAS-guarded in PR 12 |
...
- Projector crash between read and commit yields neither loss nor double application: the successor resumes from the committed checkpoint, and observation ingestion is idempotent by event ID. The discovery projector (PR 11) is the first such job: its checkpoint advances in the same transaction as the rows it projects, that transaction verifies the leader's fence against `discovery_projector_state` under `SELECT ... FOR UPDATE`, and a flush retried after a transient failure re-applies the same in-memory state without re-reading the stream.
- Endpoint cardinality admission/eviction is global and fenced: one replica cannot evict an endpoint another replica's traffic still evidences. Since PR 11 only the projector leader evicts, on the global last-seen order, inside its fenced flush.
