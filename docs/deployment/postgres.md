...
- **One deployment per database.** Every authoritative pointer and revision counter in the schema is a singleton, so a database holds exactly one deployment's state. The first boot binds the database to its `DEPLOYMENT_ID` (`greengateway.deployment_binding`); every later boot, and every one-shot command, refuses to run against a database bound to another deployment.
...
The first records the withdrawal as a committed control-plane mutation (it advances the shared security revision and appends an outbox row naming the digest); the second reclaims expired rows, bounded per run, until the membership PR makes it fenced singleton work.
