...
`client_identity_pem_path` enables mutual TLS for one physical `https` endpoint and is rejected with an `http` URL. It accepts only a mounted filesystem reference inside a pooled endpoint object; inline certificate or private-key values are not supported. At startup GreenGateway reads the file, requires a PEM certificate chain plus exactly one matching PKCS#1, PKCS#8, or SEC1 private key, and refuses to start if the file is absent, malformed, incomplete, or mismatched. The parsed identity and custom CA are applied together without changing the configured hostname, so exact-address pinning still preserves TLS SNI and hostname verification.
