...
- Refreshed every dependency of the workspace and both JavaScript projects. Most of it is invisible: 121 crates moved to their latest semver-compatible release, and the majors that needed no source change are `jsonwebtoken` 11.0 (bearer-token validation; every 11.0 breaking change was checked against the four call sites and the gateway's `Validation` and `DecodingKey` use is unaffected), `base64` 0.23, `getrandom` 0.4, `tower-http` 0.7, `syn` 3 (the structural transport guard, a build-time tool) and `rcgen` 0.14 (certificate generation, used in production only to build the gateway's own test material). Two changes are worth an operator's attention. **`rusqlite` moves 0.32 to 0.40, which bundles SQLite 3.53.2 in place of 3.46.0** for every standalone store — the audit sink, the service-token store, policy history, discovery, the principal directory and the connections store — so a deployment that keeps its SQLite files across this upgrade runs them on the newer engine; the on-disk format is unchanged and compatible in both directions, and the three behavioural changes in that range (multi-statement `execute` now refused, `u64`/`usize` binding moved behind a feature, the statement cache made optional) were audited against every store and reach none of them. **The admin UI is now built by Vite 8** with `@vitejs/plugin-react` 6 and React 19.3; the served bundle keeps its shape, with no inline script or style, so the embedded UI's `style-src 'self'; font-src 'self'` policy is unchanged. `jsonschema` and `rmcp` deliberately stay on their current major.
