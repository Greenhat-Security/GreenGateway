...
//! `(actor.user_id, actor.issuer, actor.auth_mode)`; unauthenticated requests
        ...
        if observation.routing_context_known {
            if let Some(key) = observation.routing_context_key() {
                self.routing_contexts
                    ...
                    .observe(&observation.timestamp, observation.principal.as_ref());
            }
...
impl RoutingContextAggregate {
    ...
    fn observe(&mut self, timestamp: &str, principal: Option<&PrincipalIdentity>) {
    ...
    fn load(
        ...
        for row in load_routing_principal_rows(connection)? {
            ...
            if let Some(context) = aggregate.routing_contexts.get_mut(&context_key) {
                context.principals.insert(PrincipalIdentity {
                    user_id: row.user_id,
                    issuer: row.issuer,
                    auth_method: row.auth_method,
                });
            ...
            if is_new_endpoint {
                signals.extend(self.signal_evaluator.evaluate_new_endpoint(
                    EndpointSignalObservation {
                        ...
                        user_id: principal.map(|principal| principal.user_id.as_str()),
                    },
                ));
            }
                ...
                if !is_new_endpoint && !classified.principal_seen_before {
                    signals.extend(self.signal_evaluator.evaluate_principal_new_to_endpoint(
                        PrincipalNewToEndpointSignalObservation {
                            ...
                            issuer:
                                (!principal.issuer.is_empty()).then_some(principal.issuer.as_str()),
...
fn configure_connection(connection: &Connection) -> rusqlite::Result<()> {
    ...
    ensure_discovery_endpoint_routing_principal_identity_schema(connection)?;
    ensure_discovery_endpoint_classified_signal_principal_identity_schema(connection)?;
...
fn ensure_discovery_endpoint_routing_principal_identity_schema(
    connection: &Connection,
) -> rusqlite::Result<()> {
    let columns = {
        let mut statement =
            connection.prepare("PRAGMA table_info(discovery_endpoint_routing_principals)")?;
        let columns = statement
            .query_map([], |row| {
                Ok((row.get::<_, String>(1)?, row.get::<_, i64>(5)?))
            })?
    ...
    let issuer_expression = if columns.iter().any(|(name, _)| name == "issuer") {
        "issuer"
    } else {
        "''"
    };
    let auth_method_expression = if columns.iter().any(|(name, _)| name == "auth_method") {
        "auth_method"
    } else {
        "''"
    };
    ...
    transaction.execute_batch(
        r#"
        DROP TABLE IF EXISTS discovery_endpoint_routing_principals_v2;
        CREATE TABLE discovery_endpoint_routing_principals_v2 (
            method TEXT NOT NULL,
            endpoint_template TEXT NOT NULL,
            route_host TEXT NOT NULL,
            route_path_prefix TEXT NOT NULL,
    ...
    let copy_sql = format!(
        "INSERT INTO discovery_endpoint_routing_principals_v2 \
         ...
         user_id, {issuer_expression}, {auth_method_expression} \
         FROM discovery_endpoint_routing_principals"
    );
    transaction.execute(&copy_sql, [])?;
    transaction.execute_batch(
        r#"
        DROP TABLE discovery_endpoint_routing_principals;
        ALTER TABLE discovery_endpoint_routing_principals_v2
            RENAME TO discovery_endpoint_routing_principals;
        "#,
    )?;
    transaction.commit()
}

fn ensure_discovery_endpoint_classified_signal_principal_identity_schema(
    connection: &Connection,
) -> rusqlite::Result<()> {
    let columns = {
        let mut statement = connection
            .prepare("PRAGMA table_info(discovery_endpoint_classified_signal_principals)")?;
        let columns = statement
            .query_map([], |row| {
                Ok((row.get::<_, String>(1)?, row.get::<_, i64>(5)?))
            })?
    ...
    transaction.execute_batch(
        r#"
        DROP TABLE IF EXISTS discovery_endpoint_classified_signal_principals_v2;
        CREATE TABLE discovery_endpoint_classified_signal_principals_v2 (
            method TEXT NOT NULL,
            endpoint_template TEXT NOT NULL,
            user_id TEXT NOT NULL,
            issuer TEXT NOT NULL DEFAULT '',
    ...
    let copy_sql = format!(
        "INSERT INTO discovery_endpoint_classified_signal_principals_v2 \
         (method, endpoint_template, user_id, issuer, auth_method) \
         SELECT method, endpoint_template, user_id, {issuer_expression}, \
         {auth_method_expression} FROM discovery_endpoint_classified_signal_principals"
    );
    transaction.execute(&copy_sql, [])?;
    transaction.execute_batch(
        r#"
        DROP TABLE discovery_endpoint_classified_signal_principals;
        ALTER TABLE discovery_endpoint_classified_signal_principals_v2
            RENAME TO discovery_endpoint_classified_signal_principals;
        "#,
    )?;
    transaction.commit()
}
...
mod tests {
    ...
    fn distinct_principals_separate_same_subject_by_issuer() {
        ...
        let routing_count = connection
            .query_row(
                "SELECT COUNT(*) FROM discovery_endpoint_routing_principals WHERE user_id = 'shared-subject'",
                [],
                |row| row.get::<_, i64>(0),
            )
            .expect("routing principal count should query");
        assert_eq!(routing_count, 2);
        let classified_count = connection
            .query_row(
                "SELECT COUNT(*) FROM discovery_endpoint_classified_signal_principals WHERE user_id = 'shared-subject'",
                [],
                |row| row.get::<_, i64>(0),
            )
            .expect("classified principal count should query");
        assert_eq!(classified_count, 2);
        let routing_distinct_count = connection
            .query_row(
                "SELECT distinct_principal_count FROM discovery_endpoint_routing_contexts",
                [],
                |row| row.get::<_, i64>(0),
            )
            .expect("routing distinct principal count should query");
        assert_eq!(routing_distinct_count, 2);
    }

    #[test]
    fn classified_principal_identity_survives_restart() {
        let db = TempDb::new("issuer-bound-classified-principal-restart");
        {
            let sink = aggregator_sink(&db.path);
            let mut event = observed_event(
                "POST",
            ...
            event.actor.as_mut().expect("actor should exist").issuer =
                Some("https://idp-a.example/".to_owned());
            sink.emit(&event);
            sink.flush_for_test();
        }

        {
            ...
            event.actor.as_mut().expect("actor should exist").issuer =
                Some("https://idp-b.example/".to_owned());
            sink.emit(&event);
            sink.flush_for_test();
        }
    ...
    #[test]
    fn legacy_routing_and_classified_principal_schemas_migrate_without_losing_rows() {
        let db = TempDb::new("legacy-context-principal-schemas");
        let connection = Connection::open(&db.path).expect("database should open");
        connection
            .execute_batch(
                r#"
                CREATE TABLE discovery_endpoint_routing_principals (
                    method TEXT NOT NULL,
                    endpoint_template TEXT NOT NULL,
                    route_host TEXT NOT NULL,
                    route_path_prefix TEXT NOT NULL,
                    ...
                    user_id TEXT NOT NULL,
                    PRIMARY KEY (
                        method, endpoint_template, route_host, route_path_prefix,
                        upstream_origin, user_id
                    )
                );
                INSERT INTO discovery_endpoint_routing_principals (
                    method, endpoint_template, route_host, route_path_prefix,
                    ...
                    'GET', '/legacy', 'api.example.test', '/legacy',
                    'https://legacy.internal', 'alice'
                );

                CREATE TABLE discovery_endpoint_classified_signal_principals (
                    method TEXT NOT NULL,
                    endpoint_template TEXT NOT NULL,
                    user_id TEXT NOT NULL,
                    PRIMARY KEY (method, endpoint_template, user_id)
                ...
                ) VALUES ('GET', '/legacy', 'alice');
                "#,
            )
            .expect("legacy schemas should seed");

        ensure_discovery_endpoint_routing_principal_identity_schema(&connection)
            .expect("legacy routing principal schema should migrate");
        ensure_discovery_endpoint_classified_signal_principal_identity_schema(&connection)
            .expect("legacy classified principal schema should migrate");

        let routing_identity = connection
            .query_row(
                "SELECT user_id, issuer, auth_method FROM discovery_endpoint_routing_principals",
                [],
                |row| {
                    Ok((
                        row.get::<_, String>(0)?,
            ...
            .expect("migrated routing principal should query");
        assert_eq!(
            routing_identity,
            ("alice".to_owned(), String::new(), String::new())
        );
        let classified_identity = connection
            .query_row(
                "SELECT user_id, issuer, auth_method FROM discovery_endpoint_classified_signal_principals",
                [],
                |row| {
                    Ok((
                        row.get::<_, String>(0)?,
            ...
            .expect("migrated classified principal should query");
        assert_eq!(
            classified_identity,
            ("alice".to_owned(), String::new(), String::new())
        );
    ...
    #[test]
    fn classified_principal_schema_migration_rolls_back_on_copy_failure() {
        let db = TempDb::new("invalid-classified-principal-schema");
        let connection = Connection::open(&db.path).expect("database should open");
        connection
            .execute_batch(
                r#"
                CREATE TABLE discovery_endpoint_classified_signal_principals (
                    ...
                    PRIMARY KEY (method, endpoint_template, subject_id)
                );
                INSERT INTO discovery_endpoint_classified_signal_principals (
                    method, endpoint_template, subject_id
                ) VALUES ('GET', '/invalid', 'alice');
                "#,
            )
            .expect("invalid schema should seed");

        assert!(
            ensure_discovery_endpoint_classified_signal_principal_identity_schema(&connection)
                .is_err(),
            "migration must fail closed when source identity columns cannot be copied"
        );
        let original_count = connection
            .query_row(
                "SELECT COUNT(*) FROM discovery_endpoint_classified_signal_principals WHERE subject_id = 'alice'",
                [],
                |row| row.get::<_, i64>(0),
            )
            .expect("original classified principal table should remain readable");
        assert_eq!(original_count, 1);
        let temporary_table_count = connection
            .query_row(
                "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = 'discovery_endpoint_classified_signal_principals_v2'",
                [],
                |row| row.get::<_, i64>(0),
            )
            .expect("temporary table rollback should query");
