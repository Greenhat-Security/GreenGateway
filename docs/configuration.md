...
An upstream `401` removes only the token generation used by that request so the next invocation mints again. GreenGateway does not replay the current request, even if route retry status configuration includes `401`; this prevents automatic duplication of non-idempotent operations. Authenticated Connection `401`/`403` responses become a generic `auth_failed` dependency error; their challenge headers and bodies are discarded. Token endpoint failures, bodies, challenges, URLs, client IDs, secret IDs, access tokens, and refresh tokens are excluded from caller errors and `connection.oauth_token_refresh` audit payloads. Sensitive token-response accumulation and partially parsed access/refresh fields use best-effort zeroization on success, rejection, oversize, read error, and cancellation. Failures never fall back to an anonymous upstream call.
