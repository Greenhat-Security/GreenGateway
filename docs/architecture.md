...
| 5 | IP/global rate limit and request validation | Bound pre-auth work, the method (`501`), path (`414`) and `Host` (`400`/`431`), body size, and content type, and refuse `CONNECT` outright. The method, path and host bounds are the policy kernel's own, so nothing admitted here is later refused at authorization for its shape. Trusted forwarding headers are accepted only from configured proxy CIDRs. |
