...
mod tests {
    ...
    #[test]
    fn payload_path_and_status_are_extracted_when_present() {
        let db = TempDb::new("payload-extraction");
        let sink = sqlite_sink_with_interval(&db.path, None, StdDuration::from_secs(60));

        sink.emit(&test_event(
            "audit.payload.present",
            json!({ "path": "/foo", "status": 200 }),
        ));
        sink.emit(&test_event(
        ...
        assert_eq!(present.0.as_deref(), Some("/foo"));
        assert_eq!(present.1, Some(200));
    ...
    fn query_payload_columns(
        ...
        connection
            .query_row(
                "SELECT payload_path, payload_status FROM audit_events WHERE event_type = ?1",
                params![event_type],
