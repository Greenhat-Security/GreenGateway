...
### `ADMIN_SESSION_MODE`, `ADMIN_SESSION_TTL_SECS`, `ADMIN_SESSION_MAX_ENTRIES`

Gateway-issued OIDC sessions are an opt-in server contract. `ADMIN_SESSION_MODE` defaults to `disabled`; `standalone_memory` enables a bounded, single-process store. `ADMIN_SESSION_TTL_SECS` defaults to `3600` (range `1..86400`); `ADMIN_SESSION_MAX_ENTRIES` defaults to `1024` (range `1..100000`). Set the latter two only when the mode is enabled. Restart invalidates every issued session. This mode does not persist sessions to SQLite, support multiple replicas, or use sticky routing as an HA substitute. PostgreSQL/cluster mode is rejected at startup; a shared encrypted session backend remains future work. `ADMIN_LOGIN_KEYRING` still concerns pending PostgreSQL logins and does not enable authenticated HA sessions.
